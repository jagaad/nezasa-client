...
export interface AccommodationOffersRequest {
	...
	/**
	 * Defines filtering options for accommodation offers based on suppliers.
	 * This object allows for specifying which suppliers should be included or excluded from the search results,
	 * providing more control over the source of accommodation offers.
	 */
	filters?: Filters;
}
...
export interface AccommodationBookingRequest {
	...
	/**
	 * URL of the webhook endpoint to be used to confirm an on-request booking, i.e. booking which returned a `status` of `OnRequest`
	 * In such cases, the webhook should be used to inform the caller once the on-request booking is confirmed to be either booked or unavailable.
	 */
	onRequestWebhookUrl?: string;
	metadata?: Metadata;
}
 ...
 * This object allows for specifying which suppliers should be included or excluded from the search results,
 * providing more control over the source of accommodation offers.
 * @example {"includedSuppliers":["",""],"excludedSuppliers":["",""]}
 */
export interface Filters {
	/** Offers returned can only be provided by the included suppliers. */
	includedSuppliers?: string[];
	/** Offers returned must not be provided by the excluded suppliers. */
...
export interface AccommodationSearchOffer {
	 ...
	 *
	 * AlliedTPro, Amadeus, Anixe, AOT, AtCore, ATI, Bonotel, BusyRooms, CMI, Diethelm, Eurotours, Expedia, GetARoom, Hotelbeds, Jonview, Juniper, Limatours,
	 * MTSGlobe, NWT, Peakwork, Priceline, Ratehawk, Restel, STC, Sunhotels, TBO, TourVest, Travolutionary, Travalco, Travco, W2M, Welcomebeds
	 */
	supplier: string;
	/**
	 * Offer rooms
